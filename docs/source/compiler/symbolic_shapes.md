...
The honest comparison is one question asked of every technique: when the input size changes at runtime, what actually changes in the system?
...
## 4. Why the work splits into two phases
...
None of that is unsolvable, but it is a different piece of engineering from the simple case, and it carries a performance cost the simple case does not. So the split is not arbitrary. It is the line between "each tile stands alone" and "tiles have to talk to each other".

### 4.2 Phase 1, the varying axis is an outer axis
...
Technically this is the shallow end and that is deliberate. It exercises the whole path, the bridge, the loop production, the SDSC and bundle split, the guards and the launch binding, without any of the cross-tile machinery. If the path does not work here it will not work anywhere.

### 4.3 Phase 2, the varying axis is a reduction axis
...
## 5. Where a symbol may legally live

Most coverage questions are the same question in different words. A runtime varying value can land in exactly five places, and each has a fixed answer.
...
One thing to be careful about when reading the current tree. There are guards today that refuse a symbolic stick dimension, and a matching note on granularity computation. Those belong to the earlier route, where the symbolic dimension stayed inside an operation's iteration space and work division had to divide a symbolic extent across cores. On the loop route the body's iteration space is a static tile, so those guards never see a symbol and are not evidence about this design either way.
...
We are not taking it, for one measured reason. Passing symbols to the execute node puts every dispatch on the host program correction path, which rewrites the binary before each launch at high microseconds per dispatch, per kernel, synchronously. That cost is paid on every call forever, and it is larger than the recompile cost we are trying to remove. Section 15 records this as a rejected alternative rather than an open question.

## 6. The contract

### 6.1 What we assume others provide

| Assumption | Owner | If it does not hold |
|---|---|---|
| The dynamic tensor's HBM buffer has capacity for the declared maximum | runtime | a larger runtime size overruns the buffer |
| A dimension that is not outermost in the device layout is laid out at max stride | runtime | tile addresses stop being compile-time constants |
| The backend runs a loop whose bound comes from an input argument | backend compiler | we fall back to a not-supported error, nothing miscompiles |
| The caller supplies a size that is a multiple of the granularity | consumer | we refuse the call, see Section 11 |

Core division does not appear in that list, and that is deliberate. Work division operates on the loop body, which is a static tile, so it divides a fixed extent across cores exactly as it does for a static kernel. The earlier route had to split a symbolic extent across cores and therefore had to split its granularity too. That problem does not exist once the loop is explicit.

### 6.2 The invariants

Each has a failure mode next to it, because an invariant without a failure mode is only a preference.

| # | Invariant | What breaks if violated |
|---|---|---|
| 1 | The execute node carries no symbol identifiers | the host correction path is gated exactly on that list being non-empty, so every dispatch pays high microseconds. This is the reason the project exists |
| 2 | No address or stride carries a symbol | same failure, reached from the other side |
| 3 | The trip count reaches the device as a count, however it is computed | whether the division by G happens in the bundle, in the backend, or at dispatch is an implementation choice to agree with the backend team. It is not a deep constraint, but the two sides must agree on one form |
| 4 | **The SDSC describes one tile and is fully static** | the tile is the loop body, so its geometry is the tile size. The maximum does not belong in the SDSC iteration space at all in this design. It belongs in the bundle argument and in the HBM layout |
| 5 | The runtime size is a multiple of the granularity | the tile count floor silently drops the tail and the answer is quietly wrong. We check this and refuse, we do not pad |
| 6 | One symbolic count per kernel per nesting level | two independently marked operands give two symbols and two counts, which cannot be launched. See Appendix A |
| 7 | The caller reads only the real extent of the output | the output buffer has capacity for the maximum, so anything past the real extent is whatever was in HBM |
| 8 | A tail that will be read holds the reduction's identity, or is removed by a select | multiplying by a 0/1 mask does not clean it, because NaN times zero is still NaN. See Appendix A |

Invariant 4 deserves a note, because the earlier design did the opposite. On the symbolic-SDSC route, a symbolic dimension stayed inside one op's iteration space and the SDSC was emitted at the maximum, which is what `_resolve_sdsc_size` in `superdsc.py` still does today for that route. With an explicit loop, the op inside the loop body is tile-sized and static, so nothing symbolic reaches the SDSC. That is the simplification the loop buys us.

### 6.3 The user-facing interface
...
### 6.4 The machine-facing interfaces
...
## 7. WSR 2.0, and where the symbolic bridge sits

### 7.1 What `for_each_tile` is
...
### 7.2 How it becomes a device loop
...
The first three boxes are upstream machinery we do not own. `scan` is a standard higher-order op and the WhileLoop IR already ships with PyTorch. Both WSR 1.0 and 2.0 land on the same `LoopSpec` and the same `scf.for`, so nothing is renegotiated with the backend by this change.

### 7.3 The bridge, which is the new component

Here is the part that matters. **The user never writes a `for_each_tile`.** Requiring model authors to restructure their code would kill portability, and a stock BERT or Granite would stop being stock.

Today `for_each_tile` has exactly one caller in the whole compiler, the SDPA decomposition. Automatic tiling from marked dimensions is out of scope in the WSR 2.0 first cut. So the piece that turns a marked dynamic dimension into a tiled loop does not exist yet, and building it is our main job. Ref [#4379](https://github.com/torch-spyre/torch-spyre/issues/4379), prototype in [PR #4136](https://github.com/torch-spyre/torch-spyre/pull/4136).
...
flowchart TB
  ...
  br["THE BRIDGE (ours)<br/>1. find ops whose iteration space holds a marked symbol<br/>2. assert shared axes are equal (invariant 6)<br/>3. pick the mode: map if tile-local, else refuse for now<br/>4. author for_each_tile with tile_size = G"]
...
| 2 | group the ones that share the same symbol and can run under one loop | one loop serving several operations beats one loop each |
| 3 | emit an equality check for every pair of operands that share the varying axis | without it two operands give two symbols and two trip counts, invariant 6 |
| 4 | decide the mode per operation, map if every read stays inside its own tile, otherwise refuse for now | this is the coverage decision of Section 13, made once and in one place |
| 5 | author the `for_each_tile` with `tile_size` set to the granularity, `dims` set per operand kind, and `out_dim` for map mode | the construct then lowers through machinery that already exists |

Step 4 is the one worth protecting in review. It is a single decision point, so when reduction mode is implemented later it becomes a second branch at the same place rather than a second path through the compiler.
...
```python
torch._check(x.size(0) == y.size(0))          # invariant 6, see Appendix A
...
### 8.2 Multiply by a weight vector
...
### 8.3 Multiply-accumulate, which is a linear layer
...
**Mode: map, even though this accumulates.** This is the one that usually causes an argument, so look at which axis the accumulation runs on.
...
### 8.4 Sum along the varying axis, which is Phase 2
...
### 8.5 A small network end to end
...
Every kernel gets the same trip count, resolved once per dispatch from the same runtime size, so they agree by construction. Softmax is safe here because it normalises over the last axis, which is static. Point the same softmax at the varying axis and it becomes Section 8.4.
...
No compile happens on this path. That is the whole point of the design.
...
- The WhileLoop carried state cannot survive into the bundle, `scf.for` allows none. The carry goes to a fixed buffer moved with copies, which is what Section 8.4 pays for.
...
### 9.4 The symbol's names along the way

Cross-team conversations get confused because the same value has a different name at every stage.
...
## 10. Tiling, granularity and allocation

### 10.1 One granularity for now

The first release uses a single granularity, taken straight from the annotation. No chooser runs.

A second granularity means a second binary for every kernel that touches the varying axis, which multiplies cold compile time, and cold compile time is what we are trying to reduce. Choosing a granularity automatically is an optimisation and it is covered in Section 12.

The one constraint on G that is live today is the alignment condition of Section 5.1, and it only applies if the stick axis is ever marked.

### 10.2 Why the addresses stay static
...
### 10.3 The tail, and what the caller may read
...
**Measuring the device-time cost of a range.** A kernel compiled for a range cannot make shape-specific tiling or residency choices, so some regression against a static kernel at a single shape is expected. The honest plan is to measure it per op class rather than assume it away, and to use that measurement to decide where the cost model is worth the compile time.

## 13. Op and model coverage

An op is safe under a symbolic count when it reads only inside its own tile along the varying axis. That is the whole rule, and it is exactly the map versus reduction mode question from Section 7.1.
...
| Elementwise with two independently marked operands | two symbols, two counts | needs the equality assertion of invariant 6 |
...
Worth noticing what the model already does about its own padding. It does not multiply scores by a 0/1 mask, it adds a large negative number to padded positions before softmax so they vanish after the exponent. And mean pooling divides by the sum of the mask, not by the padded length. That is invariant 8, arrived at independently by the model authors.

### 13.2 The three attention shapes
...
### 14.1 Runtime: allocation

The dynamic tensor's buffer needs capacity for the declared maximum, and for an inner symbolic dimension the layout has to be at max stride. Section 10.2 has the distinction. Ref [#2434](https://github.com/torch-spyre/torch-spyre/issues/2434).

### 14.2 Runtime: dispatch and launch
...
Backend loop support is already tracked on their side: a parent issue with children for a fixed bound, for looping by repeating programs, and for a **symbolic bound by program looping**, which is the one matching this design and is unassigned. Our ask, ref [#4397](https://github.com/torch-spyre/torch-spyre/issues/4397), describes the same capability from the torch-spyre side plus a request for an example bundle and SDSC pair. The two should be reconciled before the next backend conversation, and it is likely ours folds into theirs.
...
We generate the loop construct and ride it, we do not own it. Three contact points stay live: automatic tiling is out of scope in their first cut so the bridge is ours, the nesting and attention rework both touch the metadata we use to carry backend hints so a replacement needs agreeing (ref [#4551](https://github.com/torch-spyre/torch-spyre/issues/4551)), and hardening is in flight (ref [PR #4751](https://github.com/torch-spyre/torch-spyre/pull/4751)).
...
The order below is by dependency, not by team. Stage 1 is the only one where nothing is usable until every part of it exists.
...
### 16.1 Stage 1, functional enablement
...
| Backend compiler | a loop whose bound comes from an input argument is accepted and executed | [#4397](https://github.com/torch-spyre/torch-spyre/issues/4397), reconciled with their own issue |

Our side of this builds and proves offline against the abstract contract, so work does not stall while the backend piece is pending. What cannot be closed offline is the end to end proof, and Section 17 says why a numerical match is not enough evidence.

### 16.2 Stage 2, guard enforcement
...
### 16.4 Stage 4, optimisations

Section 12. The granularity cost model, ref [#4381](https://github.com/torch-spyre/torch-spyre/issues/4381), more than one granularity over the range, and weight residency. All of these improve a working feature and none of them is required for it to work.

### 16.5 Stage 5, extended op coverage

Reductions and matmul on the varying axis, refs [#3062](https://github.com/torch-spyre/torch-spyre/issues/3062) to [#3065](https://github.com/torch-spyre/torch-spyre/issues/3065), re-scoped from the old address framing to the loop-count route. This is the first real use of reduction mode and the point where the tail rules of Section 10.3 start to bite. Phase 2, dynamic sequence and SDPA, sits on top of it.

## 17. Acceptance, and the false green trap
...
That runs with no hardware and is how our side is proven before the backend support lands. The on-pod gate is then one compiled kernel, several real sizes, all correct, no recompile.

## 18. Risks and open items

**Ours.** The tile extent is a sympy expression and not an int, so any integer type check or interval reasoning on it will misbehave. The tail contract depends on the caller reading only what it asked for, and if a caller forgets, the wrong answer is silent, so it needs a test on their side too. The tile advance must stay in static geometry and never become a bundle symbol.

**Upstream in torch-spyre.** Automatic tiling is out of scope in the WSR 2.0 first cut, so the bridge and its entry point are ours to settle with the WSR owners. The hint-carrying metadata is being removed by the attention rework. The symbolic stick dimension gap of Section 5.1 is unticketed.
...
|---|---|---|
| `x + y`, both marked dynamic | two separate symbols, two trip counts | does not compile. `torch._check` on the sizes collapses them to one. Invariant 6 |
...
| reduce along the axis, identity in the tail | correct | invariant 8 |
...
| reduce along the axis, select | correct for NaN, Inf and 1e30 | invariant 8 |
